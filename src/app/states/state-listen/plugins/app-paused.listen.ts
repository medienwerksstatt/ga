...
export class AppPausedListen extends StateListenPluginAbstract implements StateListenPluginInterface {
    constructor(private log: LogService) {
        super();
    }

    public validate(event: StateDispatchEventInterface): boolean {
        return event.type === StateDispatchTypesEnum.APP && event.state === AppState.PAUSE;
    }

    public execute(event: StateDispatchEventInterface): void {
        this.log.log('APP.PAUSE');
    }
}
