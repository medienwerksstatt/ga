...
export class MessageReceivedListen extends StateListenPluginAbstract implements StateListenPluginInterface {
    constructor(private log: LogService) {
        super();
    }

    public validate(event: StateDispatchEventInterface): boolean {
        return event.type === StateDispatchTypesEnum.MESSAGE && event.state === MessageState.RECEIVED;
    }

    public execute(event: StateDispatchEventInterface): void {
        this.log.log('MessageReceivedListen.execute()', event.params as MessageInterface);
    }
}
