...
export class MessageReceivedDispatch extends StateDispatchPluginAbstract implements StateDispatchPluginInterface {
    ...
    public create(): BehaviorSubject<StateDispatchEventInterface> {
        this.messageService.received()
            .pipe(takeUntil(this.ngUnsubscribe), filter(message => message !== undefined))
            .subscribe((message) => this.dispatch(MessageState.RECEIVED, message));
